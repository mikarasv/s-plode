...
class ASTVisitor(c_ast.NodeVisitor):  # type: ignore
    ...
    def _default_visit(self, node: c_ast.Node, scope: FuncName) -> NodeIndex:
        node_id = self.graph.add_node(node.__class__.__name__, scope)
        return self._visit_children(node, node_id, scope)

    # ---- Specific Visitors ---- #
    def _visit_id(self, node: c_ast.ID, scope: FuncName) -> NodeIndex:
        return self.graph.add_node(node.name, scope, NodeType.ID)

    def _visit_constant(self, node: c_ast.Constant, scope: FuncName) -> NodeIndex:
        return self.graph.add_node(node.value, scope)

    def _visit_typedecl(self, node: c_ast.TypeDecl, scope: FuncName) -> NodeIndex:
        name = node.declname
    ...
    def _visit_unaryop(self, node: c_ast.UnaryOp, scope: FuncName) -> NodeIndex:
        node_id = self.graph.add_node(node.op, scope)
        return self._visit_children(node, node_id, scope)

    def _visit_binaryop(self, node: c_ast.BinaryOp, scope: FuncName) -> NodeIndex:
        node_id = self.graph.add_node(node.op, scope)
    ...
    def _visit_identifiertype(
        self, node: c_ast.IdentifierType, scope: FuncName
    ) -> NodeIndex:
        return self.graph.add_node(" ".join(node.names), scope)

    def _visit_compound(self, node: c_ast.Compound, scope: FuncName) -> NodeIndex:
        node_id = self.graph.add_node("Body", scope)
        return self._visit_children(node, node_id, scope)

    def _visit_funcdef(self, node: c_ast.FuncDef, scope: FuncName) -> NodeIndex:
    ...
    def _visit_decl(self, node: c_ast.Decl, scope: FuncName) -> NodeIndex:
        node_id = self.graph.add_node("Decl", scope)
        return self._visit_children(node, node_id, scope)

    def _visit_assignment(self, node: c_ast.Assignment, scope: FuncName) -> NodeIndex:
