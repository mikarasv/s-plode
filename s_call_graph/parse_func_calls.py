...
class FuncCallsParser:
    ...
    def _is_valid_func(self, node: NodeDict) -> bool:
        name = node["name"]
    ...
    def _get_param_idx(self, func_node_index: NodeIndex | None) -> NodeIndex | None:
    ...
    def _collect_param_names(self, param_idx: NodeIndex | None) -> set[FuncName]:
        return {
            node["name"]
    ...
    def make_params_name_unique(self) -> None:
        for func_node in filter(self._is_valid_func, self.graph.successors(0)):
            func_node_index = func_node["node_index"]
            param_idx = self._get_param_idx(func_node_index)
            param_names = self._collect_param_names(param_idx)
            self._rename_if_match(func_node_index, param_names)

    ### Function Call Params And Arguments ###
    def _is_valid_identifier_node(self, node: NodeDict) -> bool:
        return node["node_type"] == NodeType.ID

    def _make_func_var(self, node: NodeDict, edge_index: int | None) -> FuncVar:
        return FuncVar(
            var=node,
            index=edge_index if edge_index is not None else -1,
        )

    def _get_id_succs_from_node(
        ...
        for succ in self.graph.bfs_successors(node):
            if not self._is_valid_identifier_node(succ):
                continue
            ...
            seen.add(identifier)
            result.append(self._make_func_var(succ, edge_index))

        return result

    def _get_func_index(self, func_call: NodeIndex) -> NodeIndex | None:
    ...
    def _get_params_nodes(self, func_call: NodeIndex) -> list[FuncVar]:
        # Get the index of the declaration of the function referenced on the CallFunc node
        func_index = self._get_func_index(func_call)
    ...
    def _get_arg_node(self, call: NodeIndex) -> NodeIndex | None:
        node = next(
            (
    ...
    def _get_func_args(self, call: NodeIndex) -> list[FuncVar]:
        # "Expr List" (i.e the node whose childs are the arguments) on the "FuncCall" node
        arg_list_node = self._get_arg_node(call)
        all_vars = []
    ...
    def _get_body_node_in_scope(self, scope: FuncName) -> NodeIndex:
        body_node = next(
            (
                node
    ...
    def _process_func_call(self, func_call_node: NodeIndex) -> None:
        ...
        param_nodes = self._get_params_nodes(func_call_node)
        if not param_nodes:
            return

        # Childs of the "Expr List" node (i.e the arguments of the function call)
        args_nodes = self._get_func_args(func_call_node)
        if not args_nodes:
            return

        body_node = self._get_body_node_in_scope(func_call_scope)
        for param, arg in zip(param_nodes, args_nodes):
            self._add_assign_nodes(func_call_scope, body_node, param["var"], arg["var"])
